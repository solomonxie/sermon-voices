import pytest
from src.main import extract_preacher, extract_series, extract_title, extract_scripture, extract_sequence, extract_created_at
...
def test_extract_preacher(model):
    path = './blobs/华贤/4传1-93全（rar文件）/20232024/20230621传道书042（7章6节）烧荆棘的爆声.mp3'
...
@pytest.mark.parametrize("model", MODELS)
def test_extract_scripture(model):
    path = './blobs/华贤/4传1-93全（rar文件）/20232024/20230621传道书042（7章6节）烧荆棘的爆声.mp3'
    scripture = extract_scripture(path, model=model)
    assert scripture == 'Ecclesiastes ch7:v6', f"Model {model} failed"
...
@pytest.mark.parametrize("model", MODELS)
def test_extract_scripture2(model):
    path = 'blobs/唐崇荣/《唐崇荣-约翰福音》/约翰福音第01讲.mp3'
    scripture = extract_scripture(path, model=model)
    assert scripture == 'Unknown', f"Model {model} failed"
