...
def main():
    print("🔧 Starting metadata alignment...")
    ...
    for metadata_path in metadata_files:
        ...
        # 1. Align series/preacher names with translation map
        ...
        # 2. Fix "000" sequence numbers
        if metadata.get('sequence') == "000" or not metadata.get('sequence'):
            new_seq = extract_sequence_from_title(metadata.get('title', ''))
            if new_seq != "000":
                print(f"  Fixing sequence: {new_seq} extracted from '{metadata.get('title')}'")
                metadata['sequence'] = new_seq
                # Clean up titles since we have a new sequence
                metadata['title'] = cleanup_title(metadata.get('title', ''), new_seq)
                metadata['title_en'] = cleanup_title(metadata.get('title_en', ''), new_seq)
    ...
    # 5. Save updated translation cache
